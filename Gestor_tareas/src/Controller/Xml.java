...
public class Xml {
	...
	public void importarSesion(Model modelo) {
		try {
			...
			NodeList tareas = raiz.getElementsByTagName("tarea");
			NodeList correos = raiz.getElementsByTagName("correo");
			for (int i = 0; i<correos.getLength(); i++) {
				modelo.getCorreos().add(correos.item(i).getAttributes().getNamedItem("correo").getNodeValue());
			}
